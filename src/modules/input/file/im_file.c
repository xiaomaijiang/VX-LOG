...
static boolean im_file_input_open(nx_module_t *module,
    ...
    try {
                ...
                if (rv == APR_SUCCESS) {
                } else if (rv == APR_INCOMPLETE) { // partial results returned in file_info, we check the valid bitmask
                } else {
                ...
                if (((*file)->filepos > 0) && ((*file)->filepos > (*file)->size)) { // truncated, seek back to start
        ...
        if (APR_STATUS_IS_ENOENT(e.code)) {
            if ((existed == TRUE) || (imconf->filename_const == FALSE) || (apr_fnmatch_test(imconf->filename) == 0)) {
                ...
                if ((existed != TRUE) && (imconf->filename_const == TRUE)) { // do not warn every PollInterval
...
static boolean im_file_check_file(nx_module_t *module,
    ...
    try {
                ...
                if (rv == APR_SUCCESS) {
                } else if (rv == APR_INCOMPLETE) { // partial results returned. we check the valid bitmask
                } else {
                    throw(rv, "apr_stat failed on file %s", fname);
                }

                if ((*file)->inode == 0) { // no stat info stored yet (initial open failed)
                    needopen = TRUE;
...
static boolean im_file_check_files(nx_module_t *module, boolean non_active_only) {
    ...
    for (idx = apr_hash_first(pool, imconf->files);
        ...
        if (file->input == NULL) { // open files have been already checked in the previous loop
...
static nx_im_file_input_t *im_file_check_rename(nx_module_t *module,
    ...
    if (rv == APR_SUCCESS) { // finfo.valid will have all requested results
        for (idx = apr_hash_first(pool, imconf->files);
             idx != NULL;
             idx = apr_hash_next(idx)) {
                                ...
                                boolean single) //< single file only, not wildcarded
{
    ...
    if (single == TRUE) { // remove everything else
        pool = nx_pool_create_core();
        for (idx = apr_hash_first(pool, imconf->files);
             idx != NULL;
             idx = apr_hash_next(idx)) {
...
static boolean im_file_check_new(nx_module_t *module, boolean readfromlast) {
    ...
    try {
                ...
                if (apr_fnmatch_test(imconf->filename) != 0) {
                    ...
                    if (idx == NULL) { // relative path with filename only
...
static void im_file_dircheck_event_cb(nx_module_t *module) {
    ...
    if (apr_hash_count(imconf->files) >
        (unsigned int) imconf->num_open_files) { // assume we have non-active modifications
        imconf->non_active_modified = TRUE;
    }
    ...
    if (got_data == TRUE) { // force undelayed event
...
static void im_file_read(nx_module_t *module) {
    ...
    for (evcnt = 0; evcnt < IM_FILE_MAX_READ;) {
        ...
        if ((imconf->currsrc->input != NULL) &&
            ...
            got_data = TRUE;
            evcnt++;
        } else { // buffer was empty (or couldn't read a full record)
        ...
        if (got_eof == TRUE) {
            ...
            if (got_data == FALSE) {
                ...
                (file->num_eof)++;
                if (file->num_eof >=
                    2) { // if the file returns another EOF, i.e. did not recieve any data since PollInterval*2,
